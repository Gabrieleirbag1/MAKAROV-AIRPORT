...
async def banque(msg):
    ...
    async def get_rib(username):
        ...
        user_response = requests.get(f"http://192.168.1.57:8000/users/infos/banque/?username={username}")
    ...
    async def get_vol_info(numvol):
        ...
        vol_response = requests.get(f"http://192.168.1.57:8001/vols/infos/?numvol={numvol}")
    ...
    async def get_banque_info(username, numvol, rib, prix):
        ...
        banque_response = requests.get(f"http://192.168.1.57:8000/users/rib/banque/?rib={rib}")
    ...
    async def manage_response(username, numvol, rib, argent, prix, id_rib):
        ...
        if argent > prix:
            print(argent, prix)
            argent-= prix
            response = requests.put(f"http://192.168.1.57:8000/users/infos/banque/{id_rib}/", 
