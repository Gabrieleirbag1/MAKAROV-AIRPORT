...
class UserDetailApiView(APIView):
    ...
    def put(self, request, id, *args, **kwargs):
        ...
        if serializer.is_valid():
            response = requests.get(f"http://localhost:8000/users/infos/banque/?username={old_username}")
            id = response.json()[0]['id']
            response = requests.get(f"http://web-rest-4:8003/structure/infos/staff/?user_ref={old_username}")
            ...
            response = requests.put(f"http://localhost:8000/users/infos/banque/{id}/", data=json.dumps(data), headers=headers)
            
            data = {
                'user_ref': request.data.get('username', user.username),
            }
            response = requests.put(f"http://web-rest-4:8003/structure/infos/staff/{id_staff}/", data=json.dumps(data), headers=headers)
